...
def process(filename, batch_id=-1):
    pf = []
    output_path = filename + ".%d" % (batch_id) + ".pf" if batch_id!=-1 else filename+'.pf'
    import os
    ...
    with open(filename, 'r') as fp:
        ...
        mcp_data = list(np.array_split(mcp_data, NUM_BATCHES)[batch_id]) if batch_id!=-1 else list(mcp_data)
...
if __name__=='__main__':
    ...
    find_paths("beers", "triumph", ifprint=True)
    print('---------------')
    find_paths("drink", "water", ifprint=True)
#
